...
<br>
    <section class="projet-description">
      <div class="bandeau-profil" style="background-image: url('<?php $image->getImage($parcours->background, 'parcours'); ?>')">
	  ...
	  <br>
		...
		<div class="h--posts" >
		<?php $articles = $db->query('SELECT * FROM parcours ORDER BY RAND() LIMIT 4'); ?>
		<?php while($a = $articles->fetch()) { ?>
			<div class="h--posts-item revu_lifestyle revu">
					<a class="h--post-link" href="parcours.php?id=<?php echo $parcours->id; ?>"></a>
					<div class="h--post-item-img">
							<img src="<?php $image->getImage($parcours->background, "parcours"); ?>" alt="project_image">
					</div>
					<div class="h--post-desc">
							<center><h2 class="titre"><?php echo $parcours->title; ?></h2></center>
					</div>
			</div>
		<?php } ?>
	</div>
